...
describe("one-fetch disclosures and deny-rule editor", () => {
  it("offers an inactive example and toggles only deny rules", async () => {
    const wrapper = mount(UserDenyRulesEditor, {
      props: { modelValue: '{"schemaVersion":1,"rules":[]}', disabled: false },
      global,
    });
    await wrapper.find("button").trigger("click");
    const example = wrapper.emitted("update:modelValue")?.[0]?.[0] as string;
    expect(
      UserDenyRulesV1Schema.parse(JSON.parse(example)).rules[0],
    ).toMatchObject({ action: "deny", enabled: false });
    await wrapper.setProps({ modelValue: example });
    await wrapper.find("input[type=checkbox]").setValue(true);
    const toggled = wrapper.emitted("update:modelValue")?.at(-1)?.[0] as string;
    expect(
      UserDenyRulesV1Schema.parse(JSON.parse(toggled)).rules[0]?.enabled,
    ).toBe(true);
  ...
  it("locates invalid rules and refuses administrator allow actions", async () => {
    const wrapper = mount(UserDenyRulesEditor, {
      props: {
        modelValue:
          '{"schemaVersion":1,"rules":[{"id":"a","name":"Allow","enabled":true,"action":"allow","match":{}}]}',
        disabled: false,
      },
      global,
    });
  ...
  it("does not claim verified body integrity just because target metadata is signed", () => {
    ...
    const wrapper = mount(OneFetchResponseDetails, {
      props: { details, tab: "timing" },
      global,
    });
    expect(wrapper.text()).toContain("oneFetchUnverified");
    expect(wrapper.text()).not.toContain("oneFetchVerified");
    for (const phase of ["dns", "tcp/connect", "tls", "ttfb", "download"])
      expect(wrapper.text()).toContain(
        `gateway / ${phase}: oneFetchUnavailable`,
      );
    wrapper.unmount();
  });
  it("shows intermediary diagnostics and outer headers as text, never executable HTML", () => {
    const details: OneFetchResponseDetailsV1 = {
      ...
      source: "intermediary",
      outerStatus: 200,
      outerHeaders: [{ name: "Server", value: "<script>unsafe()</script>" }],
      mutations: [],
      audit: "unknown",
      integrity: "unverified",
  ...
  it("shows service/build/config versions and vendor/audit limitations", () => {
    const wrapper = mount(OneFetchCapabilities, {
