...
import type { AuthSpec, BodySpec, RequestSpecV1 } from "@xpanel/contracts";
...
<template>
  <section class="request-pane">
    <nav class="tab-list" :aria-label="$t('requestTabs')">
      <button
        v-for="item in ['params', 'headers', 'body', 'auth', 'options'] as const"
        :key="item"
        type="button"
    ...
    <div class="tab-content">
      ...
      <div v-else-if="tab === 'body'" class="body-editor">
        <div class="inline-controls">
          <select :value="bodyKind" @change="emit('bodyKind', ($event.target as HTMLSelectElement).value as BodySpec['kind'])">
          ...
          <template v-if="bodyKind === 'json'">
            <button class="ghost-button" type="button" @click="emit('prettyBody')">
              <Braces :size="14" /> {{ $t("pretty") }}
            </button>
            <button class="ghost-button" type="button" @click="emit('compactBody')">
              {{ $t("compact") }}
            </button>
          </template>
        </div>
        ...
        <KeyValueEditor
          ...
          :entry-label="$t('entry')"
        />
        <div v-else-if="current.body.kind === 'file'" class="file-option raw-file-option">
          ...
          <input v-model="current.body.mediaType" class="field" :placeholder="$t('contentTypeOptional')" />
        </div>
        <div v-else-if="current.body.kind === 'multipart'" class="multipart-editor">
          <div class="inline-controls">
            <button class="ghost-button" type="button" @click="emit('addMultipartText')">
              <Plus :size="14" /> {{ $t("textField") }}
            </button>
            <button class="ghost-button" type="button" @click="emit('addMultipartFile')">
          ...
          <div
            ...
            class="multipart-row"
          >
            <input v-model="part.enabled" type="checkbox" :aria-label="$t('enableMultipartPart')" />
            <input v-model="part.name" class="field" :placeholder="$t('fieldName')" />
            <input v-if="part.kind === 'text'" v-model="part.value" class="field" :placeholder="$t('value')" />
            <label v-else class="file-picker ghost-button">
              <FileInput :size="14" /> {{ part.file.name }}
              <input type="file" @change="emit('multipartFile', index, $event)" />
            </label>
            <button class="icon-button" type="button" :aria-label="$t('removeMultipartPart')" @click="emit('removeMultipart', index)">
              <X :size="14" />
            </button>
          </div>
          <p v-if="current.body.parts.length === 0" class="empty-note">{{ $t("multipartEmpty") }}</p>
        </div>
      ...
      <div v-else-if="tab === 'auth'" class="form-stack">
        <label>
          {{ $t("authorization") }}
          <select :value="current.auth.kind" @change="emit('authKind', ($event.target as HTMLSelectElement).value as AuthSpec['kind'])">
        ...
        <template v-if="current.auth.kind === 'basic'">
          <input v-model="current.auth.username" class="field" :placeholder="$t('username')" />
          <input v-model="current.auth.password" class="field" type="password" :placeholder="$t('password')" />
        </template>
        <input v-else-if="current.auth.kind === 'bearer'" v-model="current.auth.token" class="field" type="password" :placeholder="$t('token')" />
        <template v-else-if="current.auth.kind === 'api-key'">
          <select v-model="current.auth.location">
            ...
            <option value="cookie" disabled>{{ $t("cookieBrowserUnsupported") }}</option>
          </select>
          <input v-model="current.auth.name" class="field" :placeholder="$t('name')" />
          <input v-model="current.auth.value" class="field" type="password" :placeholder="$t('value')" />
        </template>
        <template v-else-if="current.auth.kind === 'oauth2'">
          <input v-model="current.auth.tokenType" class="field" :placeholder="$t('tokenType')" />
          <input v-model="current.auth.accessToken" class="field" type="password" :placeholder="$t('accessToken')" />
        </template>
      </div>
      <div v-else class="form-stack options-grid">
        ...
        <div v-if="hasHiddenBrowserOptions" class="compatibility-note">
          ...
          <button class="ghost-button" type="button" @click="emit('clearUnsupported')">
        ...
        <label class="check-row">
          <input
            ...
            type="checkbox"
            @change="emit('update:autoFilterBrowserHeaders', ($event.target as HTMLInputElement).checked)"
          />
        ...
        <label class="check-row">
          <input
            ...
            type="checkbox"
            @change="emit('update:persistSensitive', ($event.target as HTMLInputElement).checked)"
          />
          {{ $t("persistSensitive") }}
        </label>
      </div>
